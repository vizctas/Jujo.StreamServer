import JujoVersion from '@web/sunshine_version';

describe('JujoVersion', () => {
  test('treats stable respins as newer than the plain release', () => {
    const base = new JujoVersion('1.14.14');
    const stableRespin = new JujoVersion('1.14.14-stable.1');
  ...
  test('still orders prerelease identifiers correctly inside each channel', () => {
    expect(new JujoVersion('1.14.14-beta.2').isGreater('1.14.14-beta.1')).toBe(true);
    expect(new JujoVersion('1.14.14-stable.2').isGreater('1.14.14-stable.1')).toBe(true);
    expect(new JujoVersion('1.14.14-stable.1').isGreater('1.14.14-rc.9')).toBe(true);
  });
});
